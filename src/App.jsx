...
function LudoGame() {
    ...
    const handleDiceRoll = (value) => {
        ...
        if (possibleMoves.length === 0) {
            ...
            // Set game phase to move first, then auto-move
    ...
    const nextPlayer = () => {
        // Handle player exits first
        const playersWantingToExit = activePlayers.filter(id => players[id].wantsToExit);
        
        if (playersWantingToExit.length > 0 && activePlayers.length > 1) {
            playersWantingToExit.forEach(playerId => {
                exitPlayer(playerId);
            });
        }
        
        // Move to next active player
        const currentActiveIndex = activePlayers.indexOf(currentPlayer);
        let nextPlayerIndex = (currentActiveIndex + 1) % activePlayers.length;
