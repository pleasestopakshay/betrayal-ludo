...
function LudoGame() {
    ...
    const movePiece = (playerId, pieceId) => {
        if (playerId !== currentPlayer || gamePhase !== 'move') return;
        ...
        <div className="game-screen">
            <div className="game-header">
                ...
                <div className="dice-container">
                    <div 
                        className={`dice ${isRolling ? 'rolling' : ''} ${gamePhase !== 'roll' ? 'disabled' : ''}`}
                        onClick={rollDice}
                    >
                        {diceValue || '🎲'}
                    </div>
                        ...
                        {piecesOnCell.map((piece, idx) => (
                            <div 
                                ...
                                className={`board-piece ${piece.color} ${
                                    movablePieces.some(m => m.pieceId === piece.pieceId) && 
                                    piece.playerId === currentPlayer ? 'clickable' : ''
                                }`}
                                style={{ 
                                ...
                                onClick={() => {
                                    if (movablePieces.some(m => m.pieceId === piece.pieceId) && 
                    ...
                    if (pieceIndex !== -1) {
                        ...
                        homePieces.push(
                            <div 
                                ...
                                className={`board-piece ${player.color} ${
                                    movablePieces.some(m => m.pieceId === actualPieceIndex) && 
                                    pIndex === currentPlayer ? 'clickable' : ''
                                }`}
                                onClick={() => {
                                    if (movablePieces.some(m => m.pieceId === actualPieceIndex) && 
